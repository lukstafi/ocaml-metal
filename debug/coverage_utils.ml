...
let test_command_buffer_handlers () =
  Printf.printf "Testing command buffer handlers...\n%!";
  let device = Device.create_system_default () in
  Printf.printf "Device created\n%!";
  let queue = CommandQueue.on_device device in
  Printf.printf "Queue created\n%!";
  let cmd_buffer = CommandBuffer.on_queue queue in
  Printf.printf "Command buffer created\n%!";
...
let _Comprehensive_coverage_test =
