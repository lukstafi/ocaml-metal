...
let%expect_test "Size, Origin, and Region operations" =
  ...
  Printf.printf "Size: width=%d, height=%d, depth=%d\n" 
    size_val.width size_val.height size_val.depth;
  ...
  Printf.printf "Region: origin=(%d,%d,%d), size=(%d,%d,%d)\n"
    region_val.origin.x region_val.origin.y region_val.origin.z
    region_val.size.width region_val.size.height region_val.size.depth;
  [%expect {|
...
let%expect_test "Library and function operations" =
  ...
  CompileOptions.set_language_version compile_options
    CompileOptions.LanguageVersion.version_2_4;
...
let%expect_test "ResourceOptions and other option types" =
  ...
  let options3 = ResourceOptions.make 
    ~storage_mode:ResourceOptions.storage_mode_private
  ...
  Printf.printf "Language version set successfully\n";
  ...
  Printf.printf "Optimization level set to performance\n";
  [%expect {|
    Options created successfully
    Fast math enabled: true
    Language version set successfully
    Optimization level set to performance
  |}] 