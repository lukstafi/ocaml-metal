...
let%expect_test "Blit encoder operations" =
  ...
  let buffer_size = 40 in (* 10 floats *)
  ...
  BlitCommandEncoder.copy_from_buffer blit_encoder
    ~source_buffer ~source_offset:0
    ~destination_buffer:dest_buffer ~destination_offset:0
    ~size:buffer_size;
...
let%expect_test "Blit fill operations" =
  ...
  BlitCommandEncoder.fill_buffer blit_encoder buffer 
    { Range.location = 0; length = buffer_size } ~value:42;
...
let%expect_test "Event synchronization" =
  ...
  CommandBuffer.encode_signal_event cmd_buffer1 event signal_value;
  ...
  CommandBuffer.encode_wait_for_event cmd_buffer2 event signal_value;
...
let%expect_test "Fence synchronization in blit encoder" =
  ...
  BlitCommandEncoder.copy_from_buffer blit_encoder1
    ~source_buffer ~source_offset:0
    ~destination_buffer:dest_buffer ~destination_offset:0
    ~size:8; (* Copy only the first half *)
  ...
  BlitCommandEncoder.copy_from_buffer blit_encoder2
    ~source_buffer ~source_offset:8
    ~destination_buffer:dest_buffer ~destination_offset:8
    ~size:8; (* Copy the second half *)
...
let%expect_test "Indirect command buffer basics" =
  ...
  if supports_icb then begin
    (* Create ICB descriptor *)
    let icb_desc = IndirectCommandBufferDescriptor.create () in
    IndirectCommandBufferDescriptor.set_command_types icb_desc IndirectCommandType.concurrent_dispatch;
    ...
    let icb = IndirectCommandBuffer.on_device_with_descriptor device icb_desc
      ~max_command_count:1
      ~options:ResourceOptions.storage_mode_shared in
    ...
    (* Create data buffer *)
    let buffer_size = 16 in (* 4 floats *)
