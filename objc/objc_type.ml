...
type (_, _) tlist = [] : ('r, 'r) tlist | ( :: ) : 'a t * ('b, 'r) tlist -> ('a -> 'b, 'r) tlist
...
let byte_size_of_t : type a. a t -> int =
  ...
  | Id | Class | Sel | Ptr _ | Imp | Enc | Proto | Ivar | Unknown -> sizeof (ptr void)
...
let rec enc_of_tlist : type a b. int -> (a, b) tlist -> string =
 fun arg_offset -> function
  | [] -> ""
  | t :: xs ->
      enc_of_t t ^ string_of_int arg_offset ^ enc_of_tlist (arg_offset + byte_size_of_t t) xs

(** Returns the encoding of a method type in the Objective-C runtime. *)
let encode_method ~args return =
  enc_of_t return ^ string_of_int (byte_size_of_tlist args) ^ enc_of_tlist 0 (id :: _SEL :: args)
