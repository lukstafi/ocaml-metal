...
let method_spec ~cmd ~typ ~enc ?(runtime_lock = false) ?(thread_registration = false) imp =
  MethodSpec { cmd; typ; imp; enc; runtime_lock; thread_registration }

let ivar_spec ~name ~typ ~enc = IvarSpec { name; typ; enc }

let prop_spec ?(retain = true) ?(copy = false) ?(readonly = false) ?(notify_change = false) name
    ~typ =
